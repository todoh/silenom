 ...
 */
/**
 * VERSIÓN MEJORADA CON GESTOR DE API KEYS Y REINTENTOS
 * Llama a la API de Gemini. Si recibe un error de cuota (429),
 * automáticamente cambia a la siguiente API key del array y reintenta la petición.
 */
...
/**
 * FUNCIÓN DE API GENERAL (SIMPLE)
 * Usa la clave principal para todas las tareas que no sean de generación de imágenes pesada.
 ...
 * Se usará para TODA la generación de imágenes ('ultrasdospasos' y 'generarImagenRealistaParaMomento').
 * Si agota todas las claves, vuelve a empezar por la primera, permitiendo múltiples ciclos.
 */
async function callImageApiWithRotation(prompt, model = 'gemini-2.0-flash-preview-image-generation') {
    ...
    // --- CAMBIO 1: Definimos cuántas veces queremos ciclar por la lista de claves ---
    const RETRY_CYCLES = 3;
    
    // --- CAMBIO 2: El número máximo de intentos ahora es más alto ---
    const maxAttempts = imageApiKeys.length * RETRY_CYCLES;
    
    let lastError = null;
    // Este índice es local a la función, por lo que cada llamada empieza de cero su propio ciclo.
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
            ...
            if (response.status === 429) {
                ...
                // La rotación cíclica permite volver al inicio de la lista.
            ...
            // Si el error no es de cuota, detenemos el proceso para no gastar más llamadas.
...
// ... (el resto de tu archivo generador.js)
...
// =================================================================
// NUEVA FUNCIÓN PARA GENERACIÓN RÁPIDA DE SVG PARA DATOS
// =================================================================

/**
 * Genera un SVG y su correspondiente PNG con un flujo de dos pasos para mayor calidad.
 * 1. Generación: Crea un SVG base rápidamente a partir del prompt.
 * 2. Refinamiento: Una segunda llamada a la IA mejora el SVG inicial en detalle y estética.
...
Devuelve el resultado en un objeto JSON con la clave "svgContent".`;
        ...
        // ======================= INICIO DE LA CORRECCIÓN =======================
        // Esta es la llamada a la API que faltaba.
        const dataInicial = await callGenerativeApi(promptGeneracion, 'gemini-2.5-flash-lite', true);
        const svgInicial = dataInicial?.svgContent; // Extraemos el SVG de la respuesta
...
Devuelve ÚNICAMENTE el código SVG mejorado dentro de un objeto JSON con la clave "svgContent".`;
        ...
        const dataRefinada = await callGenerativeApi(promptRefinamiento, 'gemini-2.0-flash-lite', true);
...
/**
 * Crea un modal para que el usuario introduzca un estilo artístico.
 * Devuelve una promesa que se resuelve con el texto del estilo o se rechaza si se cancela.
 * @returns {Promise<string>} Una promesa que contiene el estilo introducido por el usuario.
 */
function obtenerEstiloDelModal() {
    return new Promise((resolve, reject) => {
        // Evita duplicar el modal si ya existe por algún error
        const existingModal = document.querySelector('.ia-modal-overlay');
        if (existingModal) {
            existingModal.remove();
        }
        ...
        input.focus(); // Pone el foco en el campo de texto automáticamente
        ...
        form.onsubmit = (e) => {
            ...
            const estilo = input.value.trim();
            closeModal();
            resolve(estilo); // Resuelve la promesa con el texto del estilo
        };
...
/**
 * EDITED: Genera una imagen contextual a partir de un texto, aceptando un parámetro de estilo opcional.
 * @param {string} userPrompt - El texto para el cual se generará la imagen.
 * @param {string} [estilo=''] - El estilo artístico opcional a aplicar.
 * @returns {Promise<{imagen: string, svgContent: null, error: string|null}>}
 */
