...
function pSetupEventListeners() {
    ...
    document.getElementById('p-copy-json-btn')?.addEventListener('click', pCopyBlueprintAsJson);
...
/**
 * Recopila el estado actual del lienzo, lo convierte a un string JSON
 * y lo copia al portapapeles del usuario.
 */
async function pCopyBlueprintAsJson() {
    try {
        // 1. Obtiene el objeto de estado actual del lienzo.
        const currentState = pGetCurrentStateObject();

        // 2. Convierte el objeto a un string JSON con formato legible.
        const jsonString = JSON.stringify(currentState, null, 2);

        // 3. Usa la API del navegador para copiar el texto al portapapeles.
        await navigator.clipboard.writeText(jsonString);

        // 4. Informa al usuario que la operación fue exitosa.
        alert('¡Plano copiado al portapapeles!');

    } catch (err) {
        // 5. Muestra un error si la copia falla.
        console.error('Error al copiar el JSON al portapapeles:', err);
        alert('No se pudo copiar el plano. Revisa la consola para más detalles.');
    }
}