let primaryApiKey = localStorage.getItem('silenosPrimaryApiKey') || "";
let imageApiKeys = JSON.parse(localStorage.getItem('silenosImageApiKeys')) || [];
...
function updateApiKey() {
    const inputElement = document.getElementById("apiInput");
    const apiKeysInput = inputElement.value.trim();

    if (!apiKeysInput) {
        alert("Por favor, ingresa al menos una API Key.");
        return;
    }

    const keysArray = apiKeysInput.split(',').map(key => key.trim()).filter(key => key);

    if (keysArray.length === 0) {
        alert("No se encontraron claves válidas.");
        return;
    }

    // Se asignan las claves al sistema nuevo
    primaryApiKey = keysArray[0];
    imageApiKeys = keysArray;
    
    // ▼▼▼ CAMBIO CLAVE ▼▼▼
    // Sincronizamos la variable antigua 'apiKey' para que los otros scripts no fallen.
    apiKey = primaryApiKey;
    // ▲▲▲ FIN DEL CAMBIO ▲▲▲

    // Guardamos en localStorage
    localStorage.setItem('silenosPrimaryApiKey', primaryApiKey);
    localStorage.setItem('silenosImageApiKeys', JSON.stringify(imageApiKeys));

    // Actualizamos la UI
    if (document.getElementById("apiKeyDisplay")) {
        document.getElementById("apiKeyDisplay").textContent = "🟢";
    }
    alert(`API Keys actualizadas.\nClave principal (para toda la app): ${primaryApiKey.substring(0, 8)}...\nTotal de claves (para rotación de imágenes): ${imageApiKeys.length}`);
}


// Actualiza la visualización de la API key al cargar la página
// (Asegúrate de que este código también se actualice)
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById("apiKeyDisplay")) {
        document.getElementById("apiKeyDisplay").textContent = primaryApiKey ? "🟢" : "[🔴]";
    }
    if(document.getElementById('apiInput')){
        // Unimos el array de claves para mostrarlo en el input
        document.getElementById('apiInput').value = imageApiKeys.join(', ');
    }
});
