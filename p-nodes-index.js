...
,ensamblador_manual: {
    title: '📝 Ensamblador Manual de Datos',
    description: 'Recopila datos de múltiples entradas y los ensambla en un único objeto de datos cuando se activa la entrada "Encendedor".',
    inputs: 7,
    inputNames: ["Nombre", "Descripcion", "Visual Prompt", "Imagen", "Etiqueta", "Arco", "Encendedor"],
    outputs: 1,
    outputNames: ["Dato Ensamblado"],
    content: `<span>Almacena los datos recibidos y los emite como un objeto al recibir señal.</span>
      <br>Nombre: Nombre del dato.<br>Descripcion: Descripción del dato.<br>Visual Prompt: Prompt para generar la imagen.<br>Imagen: URL de la imagen.<br>Etiqueta: Etiqueta del dato.<br>Arco: Arco del dato.<br> `,
    // El estado interno para guardar la última información recibida en cada entrada.
    state: {
        nombre: '',
        descripcion: '',
        promptVisual: '',
        imagen: '',
        etiqueta: 'indeterminado', // Valor por defecto
        arco: 'base'
    },
    process: (pNode, pInputs) => {
        // Inicializa el estado si es la primera vez que se ejecuta.
        if (!pNode.state) {
            pNode.state = { nombre: '', descripcion: '', promptVisual: '', imagen: '', etiqueta: 'indeterminado', arco: '' };
        }

        // 1. Actualiza el estado interno con los últimos valores recibidos.
        // Se comprueba que la entrada no sea 'undefined' para no sobrescribir un valor existente con nada.
        if (pInputs[0] !== undefined) pNode.state.nombre = pInputs[0];
        if (pInputs[1] !== undefined) pNode.state.descripcion = pInputs[1];
        if (pInputs[2] !== undefined) pNode.state.promptVisual = pInputs[2];
        if (pInputs[3] !== undefined) pNode.state.imagen = pInputs[3];
        if (pInputs[4] !== undefined) pNode.state.etiqueta = pInputs[4];
        if (pInputs[5] !== undefined) pNode.state.arco = pInputs[5];

        // 2. Comprueba si la entrada "Encendedor" (la séptima, índice 6) ha recibido una señal.
        const triggerSignal = pInputs[6];
        if (triggerSignal !== undefined && triggerSignal !== null) {
            
            // 3. Si hay señal, construye el objeto de datos con la información almacenada.
            const datoCompleto = {
                nombre: pNode.state.nombre,
                descripcion: pNode.state.descripcion,
                promptVisual: pNode.state.promptVisual,
                imagen: pNode.state.imagen,
                etiqueta: pNode.state.etiqueta,
                arco: pNode.state.arco, // Campo personalizado
                embedding: [] // Incluimos un embedding vacío por compatibilidad
            };
            
            // 4. Devuelve el objeto completo a través de su única salida.
            return datoCompleto;
        }

        // 5. Si no hay señal en "Encendedor", no produce ninguna salida (devuelve null).
        // El nodo simplemente ha actualizado su estado interno y espera.
        return null;
    }
}, gemini_text_request: {
...
salida_a_datos: {
    ...
    // ===================================================================
    // CAMBIO 1: Añade la palabra "async" aquí al principio de la línea.
    // ===================================================================
    process: async (pNode, pInputs) => { 
        const pOutputDiv = document.getElementById('p-output');
        ...
        if (!datosEntrada || typeof datosEntrada !== 'object') {
            const errorMsg = "Error: El nodo 'Salida a Datos' necesita un objeto JSON en su entrada.";
        ...
        if (typeof agregarPersonajeDesdeDatos === 'undefined') {
            const errorMsg = "Error crítico: La función 'agregarPersonajeDesdeDatos' no está disponible.";
            if(pOutputDiv) pOutputDiv.innerHTML += `<p style="color: red;">${errorMsg}</p>`;
            return;
        }
        ...
        try {
            ...
            // ===================================================================
            // CAMBIO 2: Añade la línea del "await" aquí.
            // ===================================================================
            ...
            if(pOutputDiv) pOutputDiv.innerHTML += `<hr><p style="font-weight: bold;">Proceso completado. Se añadieron ${datosArray.length} datos.</p>`;
