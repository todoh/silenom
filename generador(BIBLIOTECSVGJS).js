...
async function createUnifiedPrompt(userPrompt) {
    ...
    const elementType = await callGenerativeApi(classificationPrompt, 'gemini-1.5-flash-latest', false);
    ...
    switch (tipoElemento) {
        case 'personaje':
            specificInstructions = `
                **Filosofía de Diseño y Estilo OBLIGATORIA (SVG.js):**
                Crea un personaje **humanoide** con un estilo moderno y orgánico. Usa **proporciones anatómicas realistas y creíbles**. Las líneas deben ser suaves y definir una silueta clara.

                **Instrucciones de Dibujo OBLIGATORIAS con SVG.js:**
                1.  **Estructura y Grupos:** Organiza el cuerpo en grupos lógicos. Es la parte más importante.
                    * Crea un grupo principal: \`const personaje = draw.group().id('personaje-completo');\`
                    * Dentro de él, crea subgrupos para cada parte: \`const cabeza = personaje.group().id('cabeza');\`, \`const torso = personaje.group().id('torso');\`, etc.
                    * Dibuja las formas (paths, círculos) DENTRO de su grupo correspondiente. Ej: \`cabeza.path('M...').fill(...);\`
                2.  **Anatomía Fluida:** Usa curvas suaves para delinear el cuerpo (\`.path('... C ... S ... Q ...')\`). Las extremidades deben ser cónicas y orgánicas.
                3.  **Rostro Detallado:** El rostro es el foco. Asegúrate de que ojos, nariz y boca sean claros y expresivos.
                4.  **Vestimenta Integrada:** La ropa debe seguir las curvas del cuerpo. Usa formas superpuestas para simular pliegues.
            `;
            break;

        case 'animal':
            specificInstructions = `
                **Tipo de Elemento:** Animal (usando SVG.js).
                **Instrucciones de Dibujo OBLIGATORIAS:**
                1.  **Anatomía Específica:** Respeta rigurosamente la anatomía de la especie.
                2.  **Textura y Pelaje:** Simula pelaje, plumas o escamas usando gradientes (\`.gradient()\`) o patrones (\`.pattern()\`) con \`.fill()\`.
                3.  **Estructura:** Agrupa las partes del cuerpo (cabeza, cuerpo, patas) usando \`draw.group()\`.
            `;
            break;

        case 'planta-no-arbol':
            specificInstructions = `
                **Tipo de Elemento:** Planta/Vegetación (usando SVG.js).
                **Instrucciones de Dibujo OBLIGATORIAS:**
                1.  **Flores:** Dibuja un tallo, hojas y pétalos como formas separadas pero conectadas visualmente, agrupadas con \`draw.group()\`. Usa gradientes para el color de los pétalos.
                2.  **Arbustos:** Crea una masa densa de formas de hojas, usando colores ligeramente más oscuros para las que están "detrás" para dar profundidad.
                3.  **Estructura Botánica:** Todas las partes deben estar conectadas de forma natural.
            `;
            break;

        case 'arbol':
            specificInstructions = `
                **Tipo de Elemento:** Árbol (usando SVG.js).
                **Instrucciones de Dibujo OBLIGATORIAS:**
                1.  **Estructura Jerárquica:** Usa grupos anidados para una estructura clara.
                    * Grupo principal: \`const arbol = draw.group().id('arbol-completo');\`
                    * Estructura: \`const tronco = arbol.path(...);\`
                    * Ramas: \`const ramas = arbol.group().id('ramas');\`
                    * Follaje: \`const follaje = arbol.group().id('follaje');\`
                2.  **Conexión Orgánica:** Asegúrate de que las ramas nazcan del tronco de forma fluida.
                3.  **Follaje:** Agrupa las hojas en cúmulos (pueden ser formas \`.path()\` irregulares) alrededor de las ramas.
            `;
            break;

        case 'coche':
            specificInstructions = `
                **Tipo de Elemento:** Coche (usando SVG.js).
                **Instrucciones de Dibujo OBLIGATORIAS:**
                1.  **Estructura en Grupos:**
                    * \`const coche = draw.group().id('coche-completo');\`
                    * \`const chasis = coche.path(...).fill(...);\`
                    * \`const ruedas = coche.group().id('ruedas');\`
                    * \`ruedas.circle(...);\`
                2.  **Detalles Clave:** Incluye ruedas, ventanas, faros y luces traseras.
                3.  **Materiales y Reflejos:** Simula la pintura metálica usando \`.gradient('linear', ...)\` en el relleno del chasis.
            `;
            break;
        
        // (Se omiten otros casos por brevedad, pero seguirían la misma lógica de adaptación)

        default:
            specificInstructions = `
                **Tipo de Elemento:** ${tipoElemento} (usando SVG.js).
                **Instrucciones de Dibujo Generales:**
                1.  **Composición:** Centra el elemento principal. Usa \`.move(x, y)\` o \`.center(x, y)\`.
                2.  **Estructura:** Organiza los elementos en grupos con \`draw.group().id('nombre-descriptivo')\`.
                3.  **Volumen:** Usa \`.gradient()\` en los rellenos para dar profundidad y evitar un resultado plano.
            `;
    }

    // --- INSTRUCCIÓN FINAL COMPLETAMENTE MODIFICADA ---
    return `
        Eres un diseñador gráfico experto que genera CÓDIGO JAVASCRIPT usando la librería SVG.js.
        El objeto principal para dibujar, llamado 'draw', ya está disponible en el entorno.
        ...
        1.  Usa métodos de SVG.js como draw.rect(), draw.circle(), draw.path(), etc.
        2.  Aplica atributos usando métodos encadenados como .fill(), .stroke(), .move(), .size().
        3.  Organiza elementos complejos en grupos con draw.group() y asígnales un ID con .id().
        4.  Tu respuesta DEBE SER ÚNICAMENTE el código JavaScript. No incluyas explicaciones, la palabra "javascript", comentarios, ni bloques de código markdown (\`\`\`).

        EJEMPLO DE SALIDA PARA "un cuadrado rojo y un círculo azul":
        draw.rect(100, 100).fill('red').move(50, 50).id('cuadrado-rojo');
        draw.circle(100).fill('blue').center(200, 200).id('circulo-azul');
    `;
...
// EN: generador.js
// REEMPLAZA tu función handleGeneration() completa por esta versión.
async function handleGeneration() {
    ...
    actualizarUI(true, 'Generando concepto y código con la IA...');

    try {
        // --- INICIO DE LA NUEVA LÓGICA CON SVG.js ---

        // 1. Crear un contenedor temporal y oculto para que SVG.js dibuje.
        let tempContainer = document.getElementById('svg-temp-container');
        if (!tempContainer) {
            tempContainer = document.createElement('div');
            tempContainer.id = 'svg-temp-container';
            tempContainer.style.display = 'none'; // No es visible para el usuario
            document.body.appendChild(tempContainer);
        }
        tempContainer.innerHTML = ''; // Limpiar de generaciones anteriores

        // 2. Inicializar una instancia de SVG.js en ese contenedor.
        const draw = SVG().addTo('#svg-temp-container').size(CANVAS_WIDTH, CANVAS_HEIGHT);

        // 3. Crear el prompt que pide CÓDIGO JAVASCRIPT.
        const prompt = await createUnifiedPrompt(userPrompt);
        
        // 4. Llamar a la IA esperando TEXTO PLANO (código), no JSON.
        //    Por eso, el último argumento de callGenerativeApi es 'false'.
        actualizarUI(true, 'Ejecutando código de la IA...');
        const generatedJsCode = await callGenerativeApi(prompt, 'gemini-2.5-flash', false);

        if (!generatedJsCode) {
            throw new Error("La IA no devolvió código JavaScript.");
        }

        // 5. Ejecutar el código generado de forma segura.
        //    'new Function()' es más seguro que 'eval()'.
        //    Le pasamos el objeto 'draw' para que el código de la IA pueda usarlo.
        try {
            const generativeFunction = new Function('draw', generatedJsCode);
            generativeFunction(draw);
        } catch (e) {
            console.error("Error ejecutando el código de la IA:", e);
            console.error("Código problemático:", generatedJsCode);
            throw new Error("El código JavaScript generado por la IA no es válido.");
        }

        // 6. Una vez ejecutado el código, extraer el string XML del SVG resultante.
        const svgContent = draw.svg();
        
        // --- FIN DE LA NUEVA LÓGICA ---

        // De aquí en adelante, el flujo es el mismo que tenías, porque ya tienes el svgContent.
        ...
        fabric.loadSVGFromString(svgContent, (objects, options) => {
            ...
            group.set({ left: fabricCanvas.width / 2, top: fabricCanvas.height / 2, originX: 'center',    originY: 'bottom' });

            fabricCanvas.add(group);
            fabricCanvas.renderAll();

            // Guardamos los datos para poder usarlos en "Guardar en Datos"
            lastGeneratedSceneData = {
                nombre: userPrompt.substring(0, 30), // Usamos el prompt como nombre temporal
                descripcion: userPrompt,
                etiqueta: 'visual', // Puedes pedir a la IA que clasifique en un paso previo si quieres
                arco: 'sin_arco',
                svgContent: svgContent 
            };
            actualizarUI(false, '¡Imagen generada con éxito usando SVG.js!');
        });
 ...
 * El prompt ha sido mejorado para dar instrucciones más claras, añadir la propiedad "scale"
 * y usar un modelo de IA más potente para resultados de mayor calidad.
 * @param {string} userPrompt - La descripción textual del objeto o escena a crear.
 * @returns {Promise<string>} - Una promesa que resuelve al string JSON del modelo 3D.
 */
 ...
 * El prompt ha sido refinado para que la IA realice cambios mínimos y precisos.
 * Se ha actualizado a un modelo de IA más potente para una mejor comprensión del contexto.
 * @param {string} modeloJsonString - El string JSON del modelo 3D actual.
 * @param {string} userPrompt - La instrucción de edición del usuario (ej. "haz el coche rojo").
 * @returns {Promise<string>} - Una promesa que resuelve al string JSON del modelo 3D modificado.
 */
...
async function generarImagenSuperrealistaDesdePrompt_copiaseguridadantigua(userPrompt, modelConfig = {}) {
    if (!userPrompt) {
        throw new Error("El prompt de usuario no puede estar vacío.");
    }
...
/**
 * EDITED: This function now uses a two-step process with svg.js.
 * 1. Generate basic JavaScript code for svg.js.
 * 2. Ask the AI to refine that JavaScript code to make it more detailed.
 * 3. Execute the final JavaScript code to produce the SVG.
 * @param {string} userPrompt The user's description.
 * @returns {Promise<{imagen: string, svgContent: string}>} A promise that resolves to an object with the PNG data URL and the final SVG string.
 */
 
 
// EN: generador.js
// REEMPLAZA tu función ultras2() con esta versión con PROMPTS DE ALTA CALIDAD.
async function ultras2(userPrompt) {
    if (!userPrompt) {
        throw new Error("El prompt de usuario no puede estar vacío.");
    }
    console.log(`[Generador SVG.js de Alta Calidad] Iniciando para: "${userPrompt}"`);

    // --- PROMPT INICIAL MEJORADO (PASO 1) ---
    // Ya no pedimos algo "simple". Pedimos una base de alta calidad desde el principio.
    console.log("[Generador de Alta Calidad] PASO 1: Creando la ilustración base estructurada...");
    const promptInicialJs = `
        **Rol:** Eres un ilustrador vectorial de élite especializado en SVG.js.
        **Tarea:** Crea el código JavaScript para una ilustración bien estructurada y estéticamente agradable de "${userPrompt}".
        
        **Filosofía de Diseño OBLIGATORIA para esta base:**
        1.  **Estructura ANTE TODO:** Usa grupos (\`draw.group()\`) para organizar cada componente principal de la ilustración. Dale a cada grupo un \`.id()\`.
        2.  **Evita la Plenitud:** No uses colores planos. Usa gradientes sutiles (\`draw.gradient()\`) en los rellenos (\`.fill()\`) para dar volumen y profundidad desde el inicio.
        3.  **Define las Formas:** Usa trazos (\`.stroke()\`) con colores oscuros pero no negros puros (ej: '#333') para definir los contornos de los elementos importantes.
        4.  **Composición:** Centra horizontalmente al sujeto principal en el lienzo, pegado al bottom.
 
        5.  **Regla de Composición y Posicionamiento OBLIGATORIA:**
            * **Suelo Virtual:** Imagina que el borde inferior del SVG (la línea y=512) es el "bottom".
            * **Posicionamiento:** Dibuja el personaje DE PIE pegado a ese bottom. Los pies o la base del personaje deben tocar o estar muy cerca de y=512.
            * **Centrado Horizontal:** El personaje debe estar centrado horizontalmente, alrededor de x=256.
            * **Sin Recortes:** Asegúrate de que NINGUNA parte del personaje (especialmente la cabeza o accesorios) se salga del viewBox="0 0 512 512".

        **Reglas de Salida:**
        - El objeto para dibujar se llama 'draw'. NO lo declares, solo úsalo.
        - Responde ÚNICAMENTE con el código JavaScript, sin explicaciones ni markdown.`;

    const respuestaJsInicial = await callGenerativeApi(promptInicialJs, 'gemini-2.5-flash-lite', false);
    const codigoJsInicial = respuestaJsInicial.replace(/```(javascript|js)?/g, '').trim();

    if (!codigoJsInicial) {
        console.error("Respuesta recibida en PASO 1:", respuestaJsInicial);
        throw new Error("La IA no generó un código JS base válido en el primer paso.");
    }
    console.log("[Generador de Alta Calidad] Código JS base generado.");

    // --- PROMPT DE MEJORA MEJORADO (PASO 2) ---
    // Ahora le damos técnicas específicas de refinamiento profesional.
    console.log("[Generador de Alta Calidad] PASO 2: Refinando la ilustración con detalles profesionales...");
    const promptMejoraJs = `
        **Rol:** Eres un ilustrador vectorial experto y vas a refinar una ilustración existente en SVG.js.
        **Ilustración Base (Código JS):**
        \`\`\`javascript
        ${codigoJsInicial}
        \`\`\`
        **Tarea:** Eleva esta ilustración a un nivel profesional. Reescribe y expande el código para añadir un nivel superior de detalle y pulido.

        **Técnicas de Refinamiento OBLIGATORIAS:**
        1.  **Iluminación y Sombreado:** Añade nuevas formas (\`.path()\` o \`.polygon()\`) con rellenos oscuros o claros y opacidad baja (ej: \`.opacity(0.15)\`) para crear sombras proyectadas y brillos realistas.
        2.  **Texturizado Sutil:** Si aplica (madera, tela, metal), usa patrones (\`draw.pattern()\`) o líneas muy finas para añadir textura a las superficies.
        3.  **Refinamiento de Líneas:** Revisa los trazos. Considera variar su grosor (\`stroke: { width: ... }\`) para darles un aspecto más orgánico y dinámico, en lugar de uniforme.
        4.  **Añade Detalles Finos:** Incorpora pequeños elementos que den vida y realismo al sujeto (ej: costuras en la ropa, reflejos en los ojos, pequeñas imperfecciones).

        **Reglas de Salida:**
        - El objeto para dibujar se llama 'draw'. NO lo declares, solo úsalo.
        - Responde ÚNICAMENTE con el NUEVO CÓDIGO JavaScript completo y mejorado, sin explicaciones.`;

    const respuestaJsMejorado = await callGenerativeApi(promptMejoraJs, 'gemini-2.5-flash-lite', false); // Usamos un modelo más potente para el refinamiento
    const codigoJsFinal = respuestaJsMejorado.replace(/```(javascript|js)?/g, '').trim();

    if (!codigoJsFinal) {
        console.error("Respuesta recibida en PASO 2:", respuestaJsMejorado);
        throw new Error("La IA no generó una mejora del código JS en el segundo paso.");
    }
    console.log("[Generador de Alta Calidad] Código JS refinado con éxito.");

    // --- PASO 3: Ejecutar el código JS final (sin cambios en esta parte) ---
    console.log("[Generador de Alta Calidad] PASO 3: Ejecutando código para crear el SVG...");
    let svgFinal;
    try {
        let tempContainer = document.getElementById('svg-temp-container-batch');
        if (!tempContainer) {
            tempContainer = document.createElement('div');
            tempContainer.id = 'svg-temp-container-batch';
            tempContainer.style.display = 'none';
            document.body.appendChild(tempContainer);
        }
        tempContainer.innerHTML = '';

        const draw = SVG().addTo('#svg-temp-container-batch').size(CANVAS_WIDTH, CANVAS_HEIGHT);
        const cleanedJsCode = codigoJsFinal.replace(/^\s*(const|let|var)\s+draw\s*=.*/gm, '');
        const generativeFunction = new Function('draw', cleanedJsCode);
        generativeFunction(draw);
        svgFinal = draw.svg();

    } catch (e) {
        console.error("Error ejecutando el código JS final de la IA:", e);
        throw new Error(`El código JavaScript final generado por la IA no pudo ser ejecutado. Error: ${e.message}`);
    }

    if (!svgFinal) {
        throw new Error("La ejecución del código JS no produjo un SVG válido.");
    }

    // --- PASO FINAL: Convertir a PNG y devolver (sin cambios) ---
    console.log("[Generador de Alta Calidad] Convirtiendo SVG final a PNG...");
    const pngDataUrl = await svgToPngDataURL(svgFinal);
