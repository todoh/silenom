 ...
 */
// Archivo: geminialfa.js

/**
 * VERSIÓN FINAL CON CICLOS DE REINTENTOS
 * Llama a la IA y rota las API keys. Si llega al final de la lista,
 * vuelve a empezar desde la primera, permitiendo múltiples ciclos de intentos.
 */
/**
 * Llama a la IA para generación de contenido, con reintentos y manejo de errores de red robusto.
 * VERSIÓN REFORZADA
 */
async function llamarIAConFeedback(prompt, etapaDescriptiva, model, esJsonEsperado = true) {
    if (!imageApiKeys || imageApiKeys.length === 0) {
        throw new Error("No hay API keys configuradas. Configúralas en Ajustes.");
    }
    ...
    let response = null; // 1. Declaramos 'response' fuera del 'try' para que esté disponible en el 'catch'.

    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const currentApiKey = imageApiKeys[currentApiKeyIndex];
        const logPrefix = `[IA Feedback][Intento ${attempt + 1}/${maxAttempts}][Clave #${currentApiKeyIndex + 1}]`;

        try {
            // 2. Asignamos a la variable 'response' que está en el scope superior.
            response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${currentApiKey}`, {
                method: "POST",
            ...
            if (response.status === 429) {
                console.warn(`${logPrefix} La clave ha excedido la cuota para '${etapaDescriptiva}'. Cambiando a la siguiente...`);
                lastError = new Error(`API key con cuota excedida.`);
                currentApiKeyIndex = (currentApiKeyIndex + 1) % imageApiKeys.length;
                await sleep(500);
            ...
            if (!response.ok) {
                ...
                // Este es un error de la API (ej: 400, 500), no un error de red.
                throw new Error(`Error de API no recuperable en ${etapaDescriptiva}: ${response.status}. Detalles: ${errorDataText}`);
            }

            console.log(`${logPrefix} Petición exitosa para '${etapaDescriptiva}'.`);
            ...
            console.error(`${logPrefix} Fallo para '${etapaDescriptiva}':`, error.message);
            lastError = error;

            // 3. Lógica de decisión MEJORADA.
            // Si 'response' es null, significa que el error fue de red (como 'Failed to fetch').
            // En ese caso, no detenemos el proceso, simplemente rotamos la clave y reintentamos.
            // Si 'response' existe pero el status no es 429 (manejado arriba), entonces sí lanzamos el error
            // porque es un problema irrecuperable con la petición.
            if (response && response.status !== 429) {
                 throw lastError; // Lanza el error si fue un fallo de API (ej. 400 Bad Request)
            }

            // Si fue un error de red (response es null) o un 429, simplemente continuamos el bucle.
            currentApiKeyIndex = (currentApiKeyIndex + 1) % imageApiKeys.length;
    ...
    throw new Error(`Fallaron todos los ${maxAttempts} intentos para '${etapaDescriptiva}'. Último error: ${lastError.message}`);
}
/**
